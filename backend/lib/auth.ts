...
/** This backend is a single-operator validation harness, not a multi-tenant
 * product (see VALIDATION.md) — one shared secret is the right amount of
 * auth for every route except the OAuth callback, which Supabase itself
 * calls and can't attach our bearer token to. */
